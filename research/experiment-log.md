...
**What the audit missed.** The "3-word translation no comma" complaint is a *gloss*, not a sentence translation. Two of the 7 acquiring lemmas the user tapped that morning have run-on multi-sense glosses: `excidium → "demolition setting of the sun"` and `exordium → "beginning introduction foundation"`. Both lemmas appear on Eutropius p.1 (`Troiae excidium` in #1410, `ab exordio` in #1408). With enrichment still empty (cron-cadence gap), the lookup card was showing only the gloss — and that gloss read as a 3-word run-on phrase. The audit had probed only Eutropius sentence translations (`tiny <30 n=0`) and not lemma glosses. Plus, the manually-enriched `fere` came back with one quote whose `translation_en` was `"[Context: the passage contains 'omnibus fere annis' illustrating fere with quantities]"` — a meta-comment, not a translation. Slipped through because the verifier explicitly skips quotes (2026-05-21 spec: memory hooks, mild inaccuracy fine).

**Three code fixes (PR #157):**

1. **`parse_roma_aeterna_apkg.py` — root cause of the run-on glosses.** Line 34 was `s = TAG.sub(" ", s)` where `TAG = r"<[^>]+>"`. Anki notes wrap each English sense in block tags (`<div>demolition</div><div>setting of the sun</div>`). Collapsing every tag to one space concatenated the senses. Split into `BLOCK_TAG` (`div|br|p|li|ol|ul|tr|td|h[1-6]`, emit `"; "`) + `INLINE_TAG` (emit `" "`), with a tail collapse of `"; ;"` → `"; "` and trim of leading/trailing separators. 12 new tests in `tests/test_parse_roma_aeterna_apkg.py` pin the block-tag, inline-tag, list, table, br, entity, and macron-preservation behaviors. The source `.apkg` is offline, so this fix prevents re-occurrence on any future reparse but does not repair the prod TSV/DB — that's the operational fix below.

2. **`lemma_philology.py` QUOTES prompt + structural post-check.** Prompt (around line 193) now bans meta-commentary explicitly: "the `translation_en` field MUST be a faithful English rendering of `text`. Do NOT emit meta-commentary like `[Context: ...]`, `[This passage illustrates ...]`, `[The line shows ...]`, or any other bracketed editorial note in place of a translation. If a quote cannot be translated cleanly within ≤25 words, choose a shorter quote instead — never substitute a meta-comment." Plus `_strip_meta_commentary_quotes` — a deterministic post-check applied to the parsed payload on both the main `_enrich_batch_call` path and the `_self_correct_call` return path. Fires on prefix (`[`, `(context`, `(this passage`, `(the line`) OR substring (`illustrating`, `illustrates`, `this passage shows/contains`, `the line shows`, `the passage contains`, `[context`, `this quote`). 4 new tests in `tests/test_lemma_philology.py` cover the `fere` case + variant phrasings + the bracket-not-meta non-regression (`"the king [Roman ruler]"` and `"he is [the] king"` are NOT stripped) + defensive null/empty handling. Verifier scope unchanged (still etymology + diachrony only) — this is defense in depth against the one category the prompt can't fully prevent.

3. **`reading_intake._split_into_sentences` — Latin abbreviation protection.** The cheap splitter regex `(?<=[.!?·;\n])\s+` saw `XI Kal. Maias` and cut at `Kal.`. Now language-aware: signature is `_split_into_sentences(text: str, language_code: str = "el")`, and for `language_code='la'` it pre-protects `Kal.`/`Non.`/`Id.`/`a.d.` by swapping their `.` for a NUL sentinel before splitting then restoring on each returned part. NUL is safe as a sentinel because page text has already been control-char-stripped by `body_clean.normalize_pdf_artifacts` (`[\x00-\x08\x0B-\x1F\x7F-\x9F]`). Praenomen abbreviations (`L.`, `M.`, `A.`) held back because a single-letter `M.` can appear in other contexts; calendar terms are the proven offenders. Call site at `process_page` line 240 now passes `language_code`. 5 new tests cover the Greek default behavior (unchanged), Eutropius `Kal.` non-regression, the four calendar abbrevs, real Latin terminal split still works, and Greek is unaffected by the Latin protection.

**Companion data fix (operational, not in PR):**

`polyglot/scripts/repair_runon_glosses.py` — new one-shot LLM-judged repair tool, sibling to `regloss_lemmas.py`. Different bug class: `regloss_lemmas.py` trusts the form and replaces glosses that describe a different word; this one trusts both form AND meanings, just re-inserts missing sense separators. Heuristic: 2+ words, no comma/semicolon/period/parens/quotes/numbered-list markers, doesn't start with `to `/`I `/`the `/`a `/`an `/`of `/`in `/`by `/`for ` (verb-phrase / article-phrase prefix). Permissive on purpose; the LLM is the final arbiter. Studied-lemma-first ordering (frequency-ranked) so the lookup card damage is repaired first. Mirrors `regloss_lemmas.py` shape: `audit` subcommand writes a checkpoint JSONL; `apply` is dry-run by default, commits with `--apply`. 5 new tests in `tests/test_repair_runon_glosses.py` document the heuristic. Production run after deploy: audited 104 candidates (Codex primary + Claude failover via `llm_cli.py`); verdict histogram `{ok: 72, repair: 32}`. Applied 32 repairs including all the user-facing ones (`excidium → "demolition; setting of the sun"`, `exordium → "beginning; introduction; foundation"`, `arcus → "arch; bow; rainbow"`, `palma → "palm of the hand; blade of an oar; palm tree"`, `notus`, `fera`, `fors`, `laus`, `penna`, `protinus`, `invidia`, `foris`, `imperfectus`, `plerusque`, …). 6 of the 32 are deponent-verb glosses where the original carried morphology info (`opperior → "sum to wait for"`) that the LLM rewrote as `"to wait for (sum-conjugated)"` — readability win, mild morphology loss; acceptable since enrichment can supply morphology separately. Backup at `/opt/alif-backups/polyglot_pre_runon_20260526_125523.db`. Logged to `ActivityLog` as `gloss_runon_repair_applied`.

Also re-enriched `fere` (cleared `enrichment_*`, ran `enrich_lemma_philology.py --max-lemmas 1 --include-failed`) so the new post-check actually runs on the bad Caesar quote. Result: 3/3 quotes are now clean — `"Omnes fere Belgae ab Germanis oriundi sunt." → "Nearly all the Belgians are descended from the Germans."`, etc. No meta-commentary.

**Tests.** Full polyglot suite green (**394 passed, 1 skipped, 6 deselected** before the change, **same after the 26 additions in this PR**). New: 12 parser + 4 meta-commentary + 5 splitter + 5 heuristic = 26 new.

**Status.** PR #157 merged + deployed via `polyglot/deploy/deploy-polyglot.sh`. Data fix applied + verified. Audit doc gets an EPILOGUE pointing here.

**Lesson for future audits.** When investigating a complaint that mentions "translation," check *both* sentence translations AND lemma glosses — they're rendered side-by-side on the lookup card and the user doesn't necessarily distinguish them. The audit's `tiny (<30) n=0` probe ran only against `sentences.translation_en` and missed `lemmas.gloss_en` entirely. Updated audit + IDEAS.md flag this so the heuristic doesn't recur.
...
**Deferred (not in this PR).** (1) `Kal.` / `Non.` / `Id.` / `a.d.` abbreviation splitter that orphans Latin date fragments mid-sentence (already tracked in commit `c8b557db`). (2) Lazy enrichment on first lookup-card view (would close the cron-cadence gap; not a regression). (3) The Alif Codex hybrid migration (separate session, see `research/alif-codex-migration-plan-2026-05-26.md`). (4) Three lookup-card quality fixes surfaced when re-investigating the audit — see IDEAS.md "Polyglot Latin: three lookup-card quality fixes from the philology+translation audit": (a) `parse_roma_aeterna_apkg.py` HTML tag-stripper concatenates senses (root cause of the "3-word no comma" complaint: `excidium → "demolition setting of the sun"`); (b) `lemma_philology.py` QUOTES prompt allows meta-commentary translations (caught one bad `fere` quote); (c) the `Kal.` followup absorbed there.
...
3. **`_build_intro_cards` returns at most 6 cards TOTAL across new + rescue + textbook-preserve.** First attempt (PR #80) capped each category separately (6 + 4 + 6); user verified the next session and still got 10. Corrected in commit `a3a5044` to treat `INTRO_NEW_CARDS_PER_SESSION=6` as the total budget, with priority order new > rescue > textbook_preserve (textbook_preserve is informational only — known OCR words that get a one-time courtesy card). The previous "first-time intro cards are not capped" comment was the original design.
...
Two coupled changes to make Arabic display + transcription consistent across all review/intro cards:

**(1) `transliteration.py` — generalize bare ya/waw long-vowel inference**

The function had inferred long ā from a bare alif following a vowelless consonant for years (commit `48e9881`), but the parallel branches for long ī (bare ya) and long ū (bare waw) required an *explicit* kasra/damma on the previous consonant. In partially-vocalized Arabic the short vowel is routinely omitted before a long-vowel weak letter (the reader infers it), so 18% of stored `transliteration_ala_lc` values were wrong: `ḥadyqa` (should be `ḥadīqa`), `yjār` (should be `ījār`), `qadym` (should be `qadīm`), and similar.

Generalized the three is_long_* checks symmetrically: explicit matching short vowel OR (no short vowel and no sukun) on the previous consonant + matching weak letter. Extended the word-initial hamza-carrier branch to handle long ī/ū the same way. Added consonant-glide disambiguation: ya/waw is a consonant when it carries its own short vowel (`سِيَاسَة` = `siyāsa`, not `sīāsa`) or is immediately followed by alif/maqsura (`حَالِياً` = `ḥāliyā`, not `ḥālīā`) — Arabic phonotactics disallow two adjacent long vowels.

15 regression tests added covering bare/explicit/sukun/glide/word-initial cases. `backfill_transliteration.py` got a `--rerun-all` flag for refreshing stored values.

**(2) `vocalize_unvocalized_lemmas.py` — fill in missing tashkeel**

Production had 274 lemmas where `lemma_ar == lemma_ar_bare` (no diacritics ever stored). On intro/review cards these displayed unvocalized Arabic, and their transliterations were consonant-only ("kyk" for كيك, "twb" for توب). New script batches these through Claude CLI Haiku with a JSON schema and validates each output by comparing under `normalize_alef` after stripping diacritics — accepts hamza-form corrections (اعراب → إعراب) while rejecting genuine letter changes (plural ↔ singular).

### Production impact

- 242 lemma transliterations corrected (post-deploy) + 142 more after vocalization = **384 total**
- 2,802 `forms_translit_json` entries refreshed
- 9,280 sentence-level transliterations refreshed
- 244 lemmas vocalized (out of 274 unvocalized; remaining 30 are Hebrew/Judeo-Arabic script, Arabic numerals, or LLM-corrected variants)
- **99.0% of production lemmas (3,052/3,082) now display fully vocalized Arabic on intro/review cards**

Backup at `/opt/alif-backups/alif_pre_translit_fix_20260504_232508.db`.

### Why

User reported inconsistent transcription on review cards — `yjar` in one place, `ījār` elsewhere — and asked for both fully accented Arabic and the macron-style ALA-LC everywhere. Root cause was the missing branch in `transliterate_arabic`. Lesson: this is a textbook case of CLAUDE.md Rule #14 ("previous fix didn't fully close") — the bare-alif fix from `48e9881` set up half of a three-way symmetry; the ya/waw extension was never made.

### How to verify

- `python3 -m pytest tests/test_transliteration.py` — 15 cases, all should pass.
- Browse review/intro cards in the app: every Arabic word shows tashkeel, transcription uses macrons (ī/ū/ā) consistently.
- Spot-check via `/api/review/word/<lemma_id>`: response `transliteration` field uses macrons.

### Follow-ups

- 30 prod lemmas still unvocalized (Hebrew script, numerals, edge cases) — accept and skip.
- Consider folding `backfill_forms_translit.py` into `lemma_quality.run_quality_gates` so future imports never need a separate pass.

---

## 2026-05-04: Acquisition pipeline shows daily per-slot throughput (PR #61)

### What

Replaced the `PipelineSnapshot`-based per-box "delta today" with throughput counts derived from `ReviewLog.fsrs_log_json` box transitions plus `entered_acquiring_at` / `graduated_at`:

- New schema fields on `AcquisitionPipeline`: `box_1_in_today`, `box_2_in_today`, `box_3_in_today`, `graduated_today`. Removed `box_1_delta` / `box_2_delta` / `box_3_delta`.
- `flow_history` per-day rows extended with `box_1_in` / `box_2_in` / `box_3_in` (existing `entered` / `graduated` retained for `WordLifecycleFunnel`).
- Each box row in the Stats card shows `+N today`; new "Grad" row appears when graduations happened today; the 7-day chart now has one mini-bar row per slot (Box 1 / Box 2 / Box 3 / Graduated) with per-day counts.
- `PipelineSnapshot` model + table left in place but no longer read or written.

### Why

The "delta today" baseline was the *first time stats was loaded that day*, not midnight, so the pipeline card looked frozen even on heavy review days. User on 2026-05-04 had done 16 graduations and 22 new entries by mid-afternoon while the deltas still showed −2 / +2 / −3, which felt eternal.

Showing per-slot throughput separates "current pool size" from "how many cards passed each gate today" — Box 1 looks structurally larger because it's the holding pool, while Box 2/3 churn through quickly.

### Expected effect

- Daily progress is visible from the moment of the first review of the day.
- The 7-day per-slot chart exposes funnel imbalances (e.g. lots of Box 2 promotions but few graduations → Box 3 is the bottleneck) instead of just net `entered`/`graduated`.
...
### Problem
Commit `7ee81cf` ("Add tashkeel fading, root-aware graduation boost, and learning analysis") was a large bundled commit (24 files, 3256 insertions, 705 deletions) that legitimately added tashkeel fading and root-aware graduation, but silently reverted 10+ features across multiple files.

### Reverted features restored
1. **`sentence_selector.py`** (most impacted — 160+ lines lost):
   - Reserved intro slots (`INTRO_RESERVE_FRACTION=0.2`)
   - Within-session scaffold diversity (`SESSION_SCAFFOLD_DECAY=0.5`)
   - `compute_sentence_diversity_score()` helper
   - Rescue pass for recency-blocked words
   - Relaxed recency cutoffs (partial 2d→4h, no_idea 4h→30m)
   - `selection_info` logging on each selected sentence
   - `skip_on_demand` parameter (prefetch skips on-demand generation)
   - Function word filter in `_auto_introduce_words()`
   - Fill phase second pass for undersized sessions

2. **`schemas.py`**: `selection_info` field on `SentenceReviewItem`. `InsightsOut` restored `total_sentence_reviews`, `record_intro_day`, `record_graduation_day`; removed 4 phantom fields with no backend computation.

3. **`review.py`**: `skip_on_demand=not prefetch` in `build_session()` call.

4. **`types.ts`**: `selection_info` on `SentenceReviewItem`. Cleaned phantom InsightsData fields.

5. **`index.tsx`**: `selectionInfo` prop passed to `SentenceInfoModal`. Holistic AI explanation prompt (was changed to word-by-word).

6. **`experiment-log.md`**: 10 deleted entries (Feb 18-20) covering function word fix, selection reasoning, recency relaxation, session diversity, story zero-unknown, comprehensibility gate, LLM mapping, function word restoration, LLM audit + Claude CLI migration.

### Approach
Surgical restoration: copied pre-7ee81cf `sentence_selector.py`, then re-applied tashkeel fading (LearnerSettings, per-word show_tashkeel) and today's comprehensibility gate + SAVEPOINT fixes on top. Other files fixed with targeted edits.

### Verification
- `python3 -m pytest` — 719 tests pass (including 2 regression tests for encountered/unmapped gate)
- `selection_info` visible again in SentenceInfoModal
- Reserved intro slots, scaffold diversity, rescue pass all active again
