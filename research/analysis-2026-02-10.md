...
68% of reviews are tagged "collateral" (non-target word in the sentence). This is by design and not a concern — reading a sentence reviews ALL words equally. The user confirms comprehension of every unmarked word, regardless of why the sentence was selected. The `credit_type` field is purely metadata for analytics; it does not affect FSRS ratings.
...
### ~~Collateral credit inflation concern~~ (retracted)
**Status**: Not a concern. All words in a sentence are genuinely reviewed by the learner. Unmarked words represent real comprehension — the user is reading the sentence and choosing not to mark those words. This is the same signal as completing a story or scanning a textbook. The 68% "collateral" tag is just metadata about scheduling; it does not indicate passive or unearned credit.
...
3. **Study/textbook words underperforming**: Study-source words average 19% accuracy vs 70% for Duolingo words. The Learn mode selection algorithm may be choosing words that are too difficult, or these words need more review cycles.

4. **97 unseen words**: 34% of tracked vocabulary has never been reviewed. The 85 textbook imports need to be integrated into review sessions soon.
...
2. **Session difficulty calibration**: The declining "understood" rate suggests the greedy set cover may be selecting sentences with too many due/struggling words. Consider capping the number of difficult target words per session.

3. **Textbook word integration**: The 85 textbook scan words should begin appearing in review sessions within 1-2 days. Monitor whether the sudden influx overwhelms the session assembly algorithm.

4. **Listening mode expansion**: Only 5% of reviews are in listening mode. Given the higher comprehension rate (57% understood vs 29% for reading), listening practice with well-known words could reinforce learning.

5. **Lookup-to-review correlation**: The 488 lookup events are a rich signal. Words that are frequently looked up but still reviewed as "understood" may indicate the learner relies on lookup rather than recall. Consider tracking lookup-then-rating patterns.
