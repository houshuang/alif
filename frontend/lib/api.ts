...
export async function getStats(): Promise<Stats> {
  try {
    ...
    const stats = {
      ...
      fsrs_reviewed_today: raw.fsrs_reviewed_today ?? 0,
...
export async function submitSentenceReview(
  ...
  await enqueueReview("sentence", {
    ...
    confusion_candidate_lemma_ids: submission.confusion_candidate_lemma_ids,
    confusion_captures: submission.confusion_captures,
...
export async function scanTextbookPages(imageUris: string[], preserveKnown: boolean = false): Promise<BatchUploadResult> {
  ...
  const url = `${BASE_URL}/api/ocr/scan-pages?preserve_known=${preserveKnown ? "true" : "false"}`;
  const res = await fetch(url, {
    method: "POST",
    body: formData,
  });
