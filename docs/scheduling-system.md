...
Sentences are generated by LLM in two modes. Background/cron uses Claude Sonnet CLI (free via Max plan, ~15-30s). On-demand uses Gemini Flash (paid, ~1-2s). Falls back to Gemini if CLI unavailable.
...
For each target word:
    ...
    4. Claude Haiku CLI quality review gate (free via Max plan):
       - Checks naturalness (would a native speaker say this?)
       - Checks translation accuracy
       - Fails closed (if Claude CLI unavailable, sentence is rejected)
...
1. **Pre-generated (warm cache)**: `scripts/update_material.py` runs on cron,
   ...
   `MIN_SENTENCES=3` per word. Pool cap: 1000 active sentences with `CAP_HEADROOM=50`
   (retires down to 950 so backfill always has budget for multi-target generation).
