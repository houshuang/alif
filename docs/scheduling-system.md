...
move to Box 2 and confirm on a second UTC day before FSRS. This is the primary flow for encountered words
...
```
     ...
     Box advancement rules (2026-05-17):
       ...
       Graduation: Tiered — with distributed-day policy, early fast-track
                   requires a successful confirmation on a second UTC day
...
Graduation uses a tiered system. Tier 0/1/2 can fire on **any successful
review** (including collateral appearances in sentences targeting other
words). Tier 3 requires the word to be due
(`acquisition_next_due <= now`). All tiers require the graduating review to
have `rating >= 3`.

**Why collateral reviews count for graduation (tiers 0-2)**: Collateral
recognition is real evidence and does not need a primary/due label. With the
distributed-day policy, the evidence may come from collateral reviews but must
span a second UTC day before early graduation.

**Tier 0 — First correct**: With the distributed-day flag off, the historical
instant-graduation behavior remains. With it on, a first correct advances to
Box 2 due the next day rather than graduating.

**Distributed confirmation (2026-07-31)**: A later successful acquisition
review on another UTC day graduates immediately when the word has at least two
correct acquisition reviews and at least 80% cumulative accuracy. This adds one
spaced confirmation without forcing an otherwise redundant complete box cycle.

**Tier E — Elapsed-interval graduation (2026-07-08)**: If a correct review (rating ≥ 3, not inside the intro working-memory window) lands after at least `ELAPSED_GRADUATION_MIN_INTERVAL` (= 3 days) of real elapsed time since the *previous* review, the word graduates from **any box**. Rationale: surviving a long interval is exactly the consolidation proof the Leitner ramp exists to establish — 3 days already exceeds Box 3's interval and a fresh graduate's initial FSRS stability (S₀(Good) ≈ 2.3 d). The fixed box intervals otherwise discard this signal, so a word recognized 14 days overdue used to merely advance one box (the inverse of how the FSRS phase rewards long successful intervals). A *failed* review after a long gap means the word was forgotten, so it resets to Box 1 rather than graduating. Elapsed is measured as `now − last_reviewed` (the retention interval just demonstrated), captured before the review updates `last_reviewed`. This is the primary reason words graduate cleanly when a learner returns from a break with a pile of overdue acquiring words.
...
**Tier 1 — Perfect accuracy**: current rating ≥3, 100% cumulative accuracy,
and 3+ reviews → graduate from any box, subject to the distributed-day guard
when enabled.

**Tier 2 — High accuracy**: current rating ≥3, ≥80% cumulative accuracy, 4+
reviews, and box ≥2 → graduate, subject to the distributed-day guard when
enabled.
...
**Success gate (2026-07-25)**: Graduation never fires on rating 1 or 2.
Production history contained 2/2,106 graduations on rating 2 because cumulative
accuracy still met Tier 3; one exact regression fixture now protects the gate.
The review remains valid negative/Hard evidence and the word waits for a later
successful retrieval. See
`research/acquisition-graduation-success-gate-validation-2026-07-25.md`.

Every new acquisition log stamps
`graduation_policy_version = 2`, `distributed_day_policy_version`,
`distributed_days_confirmed`, `early_graduation_blocked`, `is_due_at_review`,
`elapsed_since_last_seconds`, and pre/post counters. These are prospective
analysis boundaries; historical rows are not backfilled.
...
```
source_bonus = 1.3 if sentence.source in {"book", "corpus"} else 1.0
```
...
quality_multiplier = 1.0   for legacy book/corpus rows with no completed review
```
...
Imported authentic rows retain neutral scoring until they receive a persisted
review. After review, both `quality_natural` and
`quality_translation_correct` must be exactly true. A completed failure or
malformed NULL verdict scores zero and is also excluded by the central
reviewability SQL gate, so another maintenance path cannot make it visible by
flipping `is_active`.
...
Sentence generation stays off the live session-build critical path. The
production default is the bounded legacy batch pipeline with Claude Sonnet for
Arabic generation and the shared Haiku-tier path for audits; the experimental
self-correct generator remains opt-in through `ALIF_USE_LEGACY_BATCH=0`.

**Single-target** (7-attempt retry loop):
```
For each target word:
    1. Build vocabulary context (known words as available scaffold)
    2. Request 1-2 sentences from LLM with:
    ...
    4. Haiku-tier quality review gate (Codex-first, then Claude/API fallback):
       - Checks naturalness (would a native speaker say this?)
       - Checks translation accuracy
       - Generation fails closed; maintenance distinguishes retryable provider failure
...
Three paths maintain sentence availability:

1. **Background warm cache (primary)**: every live non-prefetch session schedules
   `warm_sentence_cache()`, which prepares multi-target material for acquiring
   rescue, focus-cohort deficits, likely introductions, and recency-exhausted
   words. The 3-hourly `update_material.py` run is maintenance-only in
   production (`--max-step-a-sentences 0`); `refill_due_deficit.py` separately
   covers FSRS-due words outside the focus cohort with zero reviewable material.
   The safety cap is 2,000 active sentences and Step 0 retires toward 1,950.

2. **Speculative prefetch generation only**: `build_session()` can generate when
   `skip_on_demand=False`, but live user requests pass `skip_on_demand=True` for
   a DB-only response. Uncovered words are skipped for that response and filled
   by background warming for a later session.
...
**Philosophy**: the persisted, verified pool is the user-facing source of
truth. Background generation uses current learner state to keep that pool
fresh; session build itself remains fast and contains no LLM call.
...
### Scoped Authentic-Corpus Preparation and Activation

**Code**: `backend/app/services/corpus_enrichment.py`,
`backend/scripts/update_material.py --only-corpus-enrichment`
...
| `PASSAGE_MIN_TARGET_STABILITY_DAYS` | 7.0 | Automatic v2 generation targets only comfortable due lemmas, matching the supply calculation and avoiding fragile-word story bundling |
| `PASSAGE_MIN_RUNNING_WORDS` / `PASSAGE_MAX_RUNNING_WORDS` | 35 / 45 | Hard v2 running-word budget; prevents long stories from hiding low review yield behind a single card count |
| `PASSAGE_MAX_IDENTICAL_TARGET_SURFACE_USES` | 2 | Caps identical normalized target forms; any repeated selected target must use at least two mapped surface forms |
...
Cap = `0` if `high_stability_due < MIN_DUE_TARGETS`, else `clamp(high_stability_due // DUE_PER_PASSAGE, FLOOR, CEILING)`. In v2 (`clustered_short_stories_v2`), `passage_generator.py` ranks a wide pool by passage-coverage debt, restricts automatic targets to stability ≥7 days, rotates 24 story shapes, and selects exactly three semantically connected due targets so every accepted story also clears the selector's three-due-word delivery floor without a fourth-word coherence penalty. Controlled batches use one Codex planning pass to partition that pool into disjoint, storyable triples before generation. One story in every three is morphology-focused. New drafts default to three sentences and must fit 35–45 mapped running words. Any selected target that repeats must use at least two normalized surface forms, no identical form may occur more than twice, and morphology-focus stories require three occurrences in three forms. Codex `gpt-5.6-sol` performs both medium-effort drafting and the independent high-effort editorial pass with no Anthropic fallback. The previous 24 passages are negative creative context; deterministic gates reject stock/generic emotional endings and near-remakes; the editor checks causal completeness, premise consistency, target senses, Arabic naturalness, adult readability, and whether the ending is earned. Rationale and baseline: `research/maintenance-short-story-v2-2026-08-01.md`. Word-level first readout and the length/layout decision: `research/analysis-2026-08-09-recovery-and-story-efficiency.md`.
...
| `GRADUATION_MIN_CALENDAR_DAYS` | 2 | Reviews must span this many UTC calendar days for distributed confirmation and tier 3 |
...
| `ACQUISITION_GRADUATION_POLICY_VERSION` | 2 | Stamped on new acquisition reviews; v2 adds the successful-graduating-review gate and explicit replay evidence |
| `ALIF_DISTRIBUTED_DAY_GRADUATION` | enabled in production | Require a second-day success before early graduation |
| `ROOT_SIBLING_THRESHOLD` | 2 | Known root siblings needed for Easy graduation boost |
| Tier 0: first-correct | times_seen=0 + rating≥3 | Flag off: instant; flag on: Box 2 due next day |
| Distributed confirmation | ≥2 correct, accuracy≥80%, ≥2 UTC days | Graduate immediately on successful second-day evidence |
| Tier 1: perfect | current rating≥3 + accuracy=100% + seen≥3 | Graduate from any box after distributed-day guard |
| Tier 2: high accuracy | current rating≥3 + accuracy≥80% + seen≥4 + box≥2 | Graduate after distributed-day guard |
