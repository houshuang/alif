...
**Single-target** (original, 3-attempt retry loop):
```
For each target word:
    ...
    4. On failure: feed validation errors back as retry prompt
    5. Store: Sentence + SentenceWord entries + optional TTS audio
```
...
- **On-demand** (`_generate_on_demand()` in `sentence_selector.py`): groups uncovered
  words and tries multi-target first, falls back to single-target.
