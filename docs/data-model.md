...
- `review_log` — Review history: rating 1-4, mode, sentence_id, credit_type (metadata only), is_acquisition, was_confused (bool, explicit confusion signal), fsrs_log_json (pre-review snapshots for undo). Pair-level confusion candidates are currently interaction-log telemetry (`confusion_help.similar_lemma_ids` and sentence-review `confusion_candidate_lemma_ids`), not a table.
