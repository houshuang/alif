...
- `update_material.py` — Cron job (every 3h): Step 0 tier-aware cap enforcement → Step A tier-aware backfill (including conservative due-dense inactive sentence salvage before new LLM generation) → Step A2 enrich corpus sentences for active words (diacritize + translate + re-map + verify mappings via LLM, max 50/run, activates on success) → Step B audio → Step C pre-gen → Step D SAMER → Step E lemma enrichment → Step F leech reintroduction → Step G book ULK consistency → Step G1b book sentence reactivation (activates inactive book sentences on comprehensible pages) → Step G2 catch ungated lemmas (`run_quality_gates()` on lemmas with NULL `gates_completed_at`). Pipeline capped at `PIPELINE_CAP = 2000` active sentences (safety valve; lifecycle is managed by due-date tiered allocation via `pipeline_tiers.py`) (tier 1: 3 per word, tier 4: 0). Default `--model claude_sonnet` (free via Claude CLI). Falls back to Gemini if CLI unavailable.
