...
- `pipeline_stats.py` — Aggregate `data/logs/generation_pipeline_YYYY-MM-DD.jsonl`: self-correct path returned/accepted/empty counts (the path default since 2026-04-20), legacy path counts, mean sentences-per-target, top `batch_validation_failed` issues. The dominant generation path emits no events to `sentence_gen_*.jsonl`, so use this script to measure the new-path success rate. Supports `--days N`, `--date`.
