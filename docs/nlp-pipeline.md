...
Optional step in `material_generator.py` after `map_tokens_to_lemmas()`. When `settings.verify_mappings_llm` is enabled, `verify_word_mappings_llm()` in sentence_validator.py sends word-lemma pairs to an LLM for correctness checking. Gated by config (default off).
...
`build_lemma_lookup()` returns a `LemmaLookupDict` (dict subclass) that tracks collisions — cases where two different lemmas normalize to the same key (e.g., أب "father" and آب "August" both → اب). First entry wins in the lookup. When `lookup_lemma()` hits a collision key and has the pre-normalized form (`original_bare`), it uses hamza-sensitive matching then CAMeL fallback to pick the correct lemma. Collisions are logged at INFO (count) and DEBUG (details).
