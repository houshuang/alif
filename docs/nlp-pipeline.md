...
Since 2026-05-18, `/api/review/next-sentences` also performs a narrow just-in-time hardening check: selected sentence IDs whose `mappings_verified_at` predates the 2026-05-17 sense-aware resolver deploy are reverified before the session response is returned. Rows that pass or are repaired get a fresh stamp; rows that remain unsafe are removed/replaced. This avoids a full corpus sweep while ensuring future sessions do not trust old bare-form-only verification stamps.
