...
| POST | `/api/discover/add` | `{lemma_ar_bare, lemma_ar?, gloss_en?, pos?, transliteration?, register?, dialect?}` → find-or-create the canonical lemma and **introduce it immediately**, bypassing `DAILY_INTRO_CAP` (explicit user add). `register`/`dialect` persisted on the Lemma. Quality gates + material generation run in a background task (own session, no write-lock hold). Rejects gloss-less words and proper nouns with `400`. Lemmas tagged `source="dragoman"`. |
