...
1. **Front**: Arabic verse (Uthmani tashkeel, Scheherazade font, 36pt with ~1.9x lineHeight for Uthmani diacritics) pinned at top, words laid out RTL (`flexDirection: "row-reverse"`). Attribution bar shows surah name + verse number. Tap-to-lookup on individual words opens full WordInfoCard (root family, pattern chip, navigation to word detail/pattern/root pages — same component as sentence review lookup). "Show Translation" button.
