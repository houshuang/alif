...
2. **Reintro cards** — for struggling words (seen 3+ times, never recalled). Info-dense layout: large Arabic + English + transliteration hero, flow chips (POS, clickable root chip → `/root/{id}`, clickable pattern chip → `/pattern/{wazn}`), FormsStrip, etymology, memory hook mnemonic (hidden since 2026-05-22), root family with "View all" link. Single "Continue" button — no self-assessment. Backend: `_build_reintro_cards()`, max `MAX_REINTRO_PER_SESSION` (3). Struggling words also stay in the sentence selection pool, so the session includes sentences with those words for practice after the teaching card.
