...
3. Ternary ratings: understood (rating=3 for all) / partial (tap to cycle: assisted recognition=rating 2, missed=rating 1, rest=3) / no_idea (rating=1 for all). Rating 2 means retrieval failed before reveal but recognition succeeded afterward; mature FSRS words receive a soft-lapse update without an immediate retry, while acquiring words remain in their box.
...
2. **Reintro cards** — for struggling words (seen 3+ times, never recalled). Info-dense layout: large Arabic + English + transliteration hero, flow chips (POS, clickable root chip → `/root/{id}`, clickable pattern chip → `/pattern/{wazn}`), FormsStrip, etymology, memory hook mnemonic (hidden since 2026-05-22), root family with "View all" link. Single "Continue" button — no self-assessment. Continue queues `POST /api/review/reintro-result` as an acknowledgement only; it never supplies learning evidence or changes FSRS/acquisition state. The following sentence is the retrieval attempt. Backend: `_build_reintro_cards()`, max `MAX_REINTRO_PER_SESSION` (3). Struggling words also stay in the sentence selection pool.
...
3. **Lookup panel** (WordInfoCard): Shows root (clickable → `/root/{id}`), root meaning, forms strip with per-form ALA-LC transliteration, surface form transliteration for conjugated forms, pattern link (clickable → `/pattern/{wazn}`), full etymology (derivation, loanwords, cultural note), memory hooks (mnemonic hidden since 2026-05-22; cognates, collocations, usage context, fun fact still shown). Known root siblings shown as tappable pills → `/word/{id}`. If root has 2+ known siblings → prediction mode ("You know words from this root: X, Y. Can you guess the meaning?") before revealing English. Otherwise shows meaning immediately. Scrollable when content overflows.
...
5. **Back phase**: triple-tap words to cycle state: off → missed (red, rating 1) → recognized only after reveal (yellow, rating 2) → off. Builds missed_lemma_ids + confused_lemma_ids. **Confusion analysis**: when a word transitions to yellow ("did not recognize"), `GET /api/review/confusion-help/{lemma_id}` fires asynchronously. Returns up to four analysis layers: (a) morphological decomposition (color-band clitics + stem + form label), (b) form-aware visually similar words (target dictionary/surface form vs candidate dictionary/forms_json forms, edit distance + rasm skeleton + short-verb ranking, orange "Easily confused" card with reason/matched-form hints), (c) phonetically similar words (emphatic/pharyngeal confusion, purple "Sounds similar" card with confused-pair pills like ص≈س), (d) prefix disambiguation hint (green "part of root" or blue "is prefix" card for words starting with و/ف/ب/ل/ك). Analysis includes encountered and suspended words in the similarity pool, and candidate lemma IDs shown on yellow taps are carried into review telemetry when available. **Exact-surface pilot**: an eligible morphology-related yellow FSRS event is deterministically assigned control/treatment, with only one unresolved episode allowed per lemma. Treatment may reserve one later already-due reading slot for the same normalized form in a different sentence and make its canonical lemma primary; it never adds a card or changes the submitted rating/due date. Listening and acquisition are excluded.
6. **Back-phase actions**: "Know All" (understood) / "Continue" (partial, if words marked) / "No idea" (no_idea)
7. **Back/Undo**: after submitting, can go back to previous card — undoes the review (restores pre-review FSRS state via backend undo endpoint), removes from sync queue if not yet flushed, restores word markings, and removes/reopens exact-surface pilot trigger/outcome metadata tied to the deleted review
...
## Learn Mode
1. **Pick phase**: Shows 5 candidate words one at a time with info-dense scrollable card. **Hero section**: large Arabic word, English gloss, transliteration, flow chips (POS, CEFR level, frequency rank, tappable root chip → `/root/{id}`, tappable pattern chip → `/pattern/{wazn}`), FormsStrip with transliterations. **Info sections** (scrollable, only shown if data exists): memory hook (mnemonic hidden since 2026-05-22), etymology (derivation + pattern), cross-language cognates, root family with "View all" link, pattern examples (up to 4 sibling words with knowledge-state dots) with "View all" link, usage context, fun fact/cultural note. No play button (word-level TTS disabled).
...
1. **Front**: Arabic verse (Uthmani tashkeel, Scheherazade font, 36pt with ~1.9x lineHeight for Uthmani diacritics) pinned at top, words laid out RTL (`flexDirection: "row-reverse"`). Attribution bar shows surah name + verse number. All words tappable — content words open full WordInfoCard (root family, pattern chip, navigation to word detail/pattern/root pages — same component as sentence review lookup), function words show gloss-only card. Prev/next arrows navigate between tapped words. Persistent highlights on looked-up words, removable by re-tapping. ActionMenu (Ask AI, flag lemma, refresh) available on ProgressBar. Uses shared `tappedOrder`/`tappedCursor`/`tappedCacheRef` infrastructure from sentence review. "Show Translation" button.
2. **Back**: Arabic stays visible (30pt), English translation (Sahih International) + ALA-LC transliteration (via `transliterate_arabic()` service, not the junk phonetic data from risan/quran-json) appear below gold divider. Looked-up words shown as summary pills. Three rating buttons: "Got it" (green) / "Partially" (amber) / "Not yet" (red).
3. **SRS**: Simple level-based (not FSRS). Level 0=unseen, 1-7=learning, 8=graduated. "Got it" advances level with intervals: 1d→3d→7d→14d→30d→60d→graduated. "Partially" drops 1 level, due in 2h. "Not yet" resets to level 1, due immediately. "Got it" button also available on front side (skip translation for known verses).
4. **Gating**: 2 new verses per session (max 5 total). Only introduced when non-understood backlog < 20. Only lemmatized verses can be introduced (ensures word data available for lookup).
5. **Lemmatization**: Lazy — `lemmatize_quran_verses()` processes next 20 verses when <10 lemmatized unseen remain. Uses existing pipeline (tokenize → lemma lookup → CAMeL fallback → LLM batch translation for unknowns). **Ta maftouha fallback**: when lookup fails, tries replacing word-final ت with ة and re-lookup — handles ~15 high-frequency Quranic spellings (رحمت/رحمة, نعمت/نعمة, etc.). **Hamzat al-wasl fallback**: `_hamzat_wasl_lookup()` restores dropped initial alef after proclitic stripping (بسم → ب + اسم). New Quran lemmas get general Arabic glosses + root detection + auto-enrichment (forms/etymology) via `_create_unknown_quran_lemmas()`. **Gloss guarantee**: 6-layer pipeline in `select_verse_cards()` ensures every word gets a gloss: function word dict → pronoun suffix decomposition → DB proclitic lookup → morphological lookup (38K forms) → LLM batch translation → transliteration fallback. Tests: `test_gloss_coverage.py`.
