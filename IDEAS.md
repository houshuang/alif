...
- [DEFERRED — re-evaluate late Feb 2026] **Sentence utilization**: 41% of active sentences never shown (229/560) as of Feb 16. Mostly pre-generated for words not yet due. Will naturally improve as more words enter review. Re-evaluate when system has been running 4+ weeks — if never-shown rate is still >30%, consider: (a) reduce pre-generation, rely more on JIT on-demand, (b) retire never-shown sentences after 14 days, (c) run `rotate_stale_sentences.py` on cron.
