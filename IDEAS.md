...
- **Same-day FSRS review dampening**: Skip or dampen FSRS stability updates for words already reviewed the same calendar day. Currently scaffold words (non-target words in reviewed sentences) get credited every time, leading to 60+ same-day reviews with zero stability growth (FSRS needs inter-day spacing to grow stability). Possible approaches: (a) skip FSRS update if word reviewed today, (b) only credit target words with full FSRS, give scaffold words a lighter "seen" update, (c) track last-review-date per word and skip same-day re-reviews.
- **Sentence utilization tracking**: 41% of active sentences never shown (229/560). Generation outpaces consumption. Consider: (a) reduce generation when word already has 2+ unused sentences, (b) prefer reusing existing sentences before generating new, (c) retire never-shown sentences after N days.
