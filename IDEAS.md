...
- [DONE 2026-08-03] Expanded the page-68 follow-up into a complete three-page Story #240 review rather than applying the verifier's first 14 suggestions. The exact-preimage manifest covers 93 reviewed tokens, 83 mapping targets, and 13 contextual glosses; reuses gated feminine `الَّتِي`, interrogative `أَيّ`, `مَلِك`, `قَوْم`, and other safe destinations; creates 47 distinct identities through full synchronous quality gates; and repairs nine malformed existing entries. A full production-snapshot rehearsal changed 81 mappings, created no learner rows, preserved every new identity as canonical, and passed SQLite quick-check. The processed-book importer now lets curated mappings override heuristic homographs and treats any non-curated verifier change as a fail-closed curation blocker rather than an automatic write.
