...
- **[OPEN]** Port Alif's *generation-side* post-gen over-exposure reject+retry (`sentence_generator._check_scaffold_diversity`, `DIVERSITY_SENTENCE_THRESHOLD≈10`) into `material_generator.batch_generate_material` — drop generated candidates whose scaffold contains a word already in ≥N active sentences, relying on the over-request buffer (no 7× retry loop; polyglot generation is batched). Grows corpus diversity so the confirmation sweep can reach the long-tail rare unconfirmed words not yet present in any sentence. Deferred from the 2026-06-01 confirmation-sweep PR to keep the generation-path change separate from the selection rewrite.
