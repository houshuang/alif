...
- **Comprehensibility gate** — sentences must have ≥60% known **scaffold** words (non-due, non-function content words) to be shown in review. Acquiring words only count as "known" if past box 1 (stability ≥ 0.5). This prevents book-imported sentences from appearing when most words are freshly batch-imported. All words are learnable (no function word exclusions).
...
- **Sentence pipeline cap**: Hard cap of 300 active sentences, enforced by `update_material.py` Step 0 (retires to 270 via CAP_HEADROOM=30, leaving budget for multi-target backfill; priority: never-shown stale → shown stale → oldest, min 1 per word). MIN_SENTENCES=2 target for backfill generation. JIT on-demand generation (MAX_ON_DEMAND=10/session) fills gaps with current vocabulary. Cron runs `rotate_stale_sentences.py` (diversity-based retirement) then `update_material.py` (cap enforcement + backfill + audio + pre-gen) every 6h.
...
- **All import paths must run quality gate** — `import_quality.classify_lemmas()` filters junk AND classifies words by category (standard/proper_name/onomatopoeia). Sets `Lemma.word_category` on import. Proper names get gloss prefixed with "(name)". Names/sounds are deprioritized in scheduling and never auto-introduced. Integrated in OCR, story import, and book import paths.
- **Every sentence_word must have a lemma_id** — all 5 sentence storage paths (material_generator, sentence_selector on-demand, generate_sentences_claude, store_multi_target_sentence) reject sentences with any unmapped word. Exception: book_import keeps sentences with unmapped tokens (`lemma_id=None`) since they're still useful for reviewing mapped words. Mapping uses `build_comprehensive_lemma_lookup()` (ALL lemmas, not just learned). Function words all have Lemma DB entries (created by `backfill_function_word_lemmas.py`). Validation still uses restricted learned-only lookup.
...
- `material_generator.py` — Orchestrates sentence + audio generation for a word. Dynamic difficulty via `get_sentence_difficulty_params()`. Default needed=2 (warm cache), requests needed+2 to absorb validation failures. JIT on-demand generation in session builder uses current vocabulary for fresher sentences. `store_multi_target_sentence()` for saving multi-target generated sentences with correct SentenceWord mappings. `warm_sentence_cache()` — background task that pre-generates sentences using **multi-target generation** for focus cohort gaps + likely auto-introduction candidates; respects 300-sentence pipeline cap (allows up to 310). **NULL lemma_id guard**: all sentence storage paths reject sentences with any unmapped word (uses `build_comprehensive_lemma_lookup()` for mapping).
- `acquisition_service.py` — Leitner 3-box (4h→1d→3d). **Two-phase advancement**: box 1→2 always allowed (encoding), box 2→3 and graduation require `acquisition_next_due <= now` (consolidation). Graduation: box≥3 + times_seen≥5 + accuracy≥60% + reviews on ≥2 UTC calendar days. `GRADUATION_MIN_CALENDAR_DAYS=2`. `start_acquisition()` accepts `due_immediately=True` for auto-introduced words. **Preserves existing meaningful source** (book, duolingo, textbook_scan, etc.) — only overwrites generic "study"/"encountered" sources.
...
| GET | `/api/stories/{id}/pages/{page}` | Book page detail: words + sentences |
...
- `lemmas` — Dictionary forms: root FK, pos, gloss, frequency_rank, cefr_level, grammar_features_json, forms_json, example_ar/en, transliteration, audio_url, canonical_lemma_id (variant FK), source_story_id, word_category (NULL=standard, proper_name, onomatopoeia), thematic_domain, etymology_json, memory_hooks_json
- `user_lemma_knowledge` — Per-lemma SRS state: knowledge_state (encountered/acquiring/new/learning/known/lapsed/suspended), fsrs_card_json, times_seen, times_correct, total_encounters, source (study/duolingo/textbook_scan/book/story_import/auto_intro/collateral/leech_reintro — preserved through acquisition, not overwritten), variant_stats_json, acquisition_box (1/2/3), acquisition_next_due, graduated_at, leech_suspended_at, leech_count
