...
- **Lemmatization feedback loop** — when a word_mapping flag is fixed: (1) if correct lemma missing from DB, auto-creates it as `source=flag_autocreate` with encountered state, (2) propagates fix to other active sentences with same bad mapping (LLM-verified per sentence, max 50). At generation time, `disambiguate_mappings_llm()` resolves ambiguous tokens (collisions/multi-clitic) using sentence context.
