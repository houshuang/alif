...
- **No lemmas without English gloss** — All lemma creation paths (`_import_unknown_words`, `ocr_service`) skip creating lemmas when LLM translation returns empty `gloss_en`. A word without a translation is useless for learning. `warm_sentence_cache` Phase 5 auto-backfills any that slip through (up to 10/run). Backfill script: `scripts/backfill_empty_glosses.py`.
