...
- **Lemmatization feedback loop** — Three layers: (1) **Generation-time correction**: `verify_and_correct_mappings_llm()` catches wrong mappings and corrects them inline (finds/creates correct lemma via `correct_mapping()`), so sentences are kept instead of discarded. (2) **User flag resolution**: when a word_mapping flag is fixed, auto-creates missing lemmas (`source=flag_autocreate`), propagates fix to other active sentences (LLM-verified, max 50). (3) **Disambiguation**: `disambiguate_mappings_llm()` resolves ambiguous tokens (collisions/multi-clitic) using sentence context.
