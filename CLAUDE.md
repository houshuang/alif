...
- **LLM**: All text generation via Claude CLI (free via Max plan). Sonnet for sentence gen (87% pass rate), Haiku for quality gate + enrichment + tagging + flags + disambiguation. Story gen: Claude Opus (retry loop). **Latency-sensitive paths use Anthropic API directly** (`model_override="anthropic"` in `llm.py`) — CLI subprocess startup adds ~2-3s which is unacceptable for interactive UX. Current direct-API paths: interactive chat (`/api/chat/ask`). **Do NOT change these to CLI without asking** — the speed difference matters for user-facing interactions. API fallback chain (when CLI unavailable): GPT-5.2 → Claude Haiku API. Gemini used ONLY for vision/OCR (`ocr_service.py`). Keys: GEMINI_KEY (OCR only), OPENAI_KEY, ANTHROPIC_API_KEY in `.env`
- **Claude Code CLI**: `claude -p` wrapper for free LLM via Max plan. Primary LLM backend for ALL batch/background text tasks — integrated into `llm.py` as `claude_sonnet`/`claude_haiku` model overrides (also default when no override specified). Also: standalone `generate_structured()` + `generate_with_tools()` in `claude_code.py`. See `docs/backend-services.md`.
...
- **LLM Cost Tracking**: All litellm calls auto-logged via `limbic.cerebellum.cost_log` callback registered in `main.py` lifespan. Shared SQLite DB at `/opt/limbic-data/llm_costs.db` on Hetzner (also used by petrarca). Sync to local: `python -m limbic.cerebellum.cost_log sync`. Reports: `python -m limbic.cerebellum.cost_log report --days 7`.
...
- **All import paths must call `run_quality_gates()`** — centralized post-creation pipeline in `lemma_quality.py`. Single function replaces the old scattered calls to `finalize_new_lemmas()`, variant detection, and enrichment. Runs in order: (1) finalize — cleans bare forms (both `lemma_ar` and `lemma_ar_bare`), assigns `frequency_rank` from cached CAMeL MSA data (10.5M forms, lazy-loaded), flags duplicate canonical lemmas; (2) variant detection — `detect_variants_llm()` + `detect_definite_variants()` + `mark_variants()`; (3) enrichment — forms, etymology, transliteration (background thread by default); (4) stamps `gates_completed_at` timestamp. **Model-level guard**: `select_next_words()` and `_build_reintro_cards()` filter out lemmas where `gates_completed_at IS NULL` — ungated lemmas never appear in sessions. **Cron catchup**: `update_material.py` Step G2 finds and gates any lemmas that slipped through. Tests in `test_gloss_coverage.py` enforce all import paths call `run_quality_gates`. Pre-creation quality gate (`import_quality.classify_lemmas()`) still runs separately in each import path to filter junk before creating Lemma records. **Also cleans dirty bare forms**: the LLM returns a `cleaned_arabic` field when it detects OCR artifacts (ال-prefix baked in, ه→ة). OCR service applies this before creating Lemma records. The LLM correctly distinguishes dirty OCR (المطحونه→مطحونة) from legitimate ال-integral words (الله, الذي, التقى, الرازي). `clean_bare_form()` in `lemma_quality.py` is punctuation-only — ال stripping is too ambiguous for rule-based automation.
...
- `backend/app/services/lemma_quality.py` — Centralized lemma quality gate: `run_quality_gates()` pipeline (finalize + variants + enrich + stamp `gates_completed_at`)
...
- `backend/scripts/generate_repetition_podcasts.py` — Repetition-focused podcasts targeting acquiring words (3-4x repetition per target word, known words as scaffold)
