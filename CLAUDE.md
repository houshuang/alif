...
- **Offline**: Frontend queues reviews in AsyncStorage sync queue, bulk-syncs via POST /api/review/sync when online. Sessions cached per mode with 30-minute staleness TTL (stale sessions discarded, rebuilt fresh). Story word lookups persisted in AsyncStorage. At 3 cards remaining, frontend triggers both `warm-sentences` (background sentence pre-generation) and session prefetch.
