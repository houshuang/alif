...
- **Verb conjugation recognition** — `build_lemma_lookup()` Pass 3 generates all standard Arabic verb conjugation forms (past suffixes + present prefix/suffix combinations) for verbs with `present` in forms_json. Allows the comprehensibility gate to recognize يكتبون as a known form of كتب. ~33 forms per verb.
