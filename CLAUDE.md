...
- **Authentic corpus enrichment is always scoped and two-phase** — use `update_material.py --only-corpus-enrichment` (or the explicitly opted-in full-workflow Step A2) with `--kind` and/or repeatable `--corpus-sentence-id`; kind and IDs intersect. The service runs under the shared material-update flock, recovers legacy claim sentinels only inside that exact scope, accepts at most 50 enrichment rows (default 20), verifies letter-preserving substantial tashkīl, translation, final mappings, canonical target bookkeeping, and authentic-row quality, then leaves successful rows inactive. Preparation and activation limits must never both be nonzero in one invocation: activate only with `--corpus-limit 0`. Activation defaults to zero, is separately capped at 20, rechecks fresh canonical demand immediately before visibility, blocks any sentence containing acquiring content, respects an explicit active-pool ceiling, and never retires other material automatically. Never enable unscoped global corpus processing or bulk-reset the generic sentinel backlog.
