...
- **All import paths must call `finalize_new_lemmas()`** — centralized post-creation gate in `lemma_quality.py` that: (1) cleans bare forms (strip punctuation artifacts), (2) assigns `frequency_rank` from cached CAMeL MSA data (10.5M forms, lazy-loaded), (3) flags potential duplicate canonical lemmas (logged, not auto-merged), (4) warns about empty glosses. Wired into story_service, ocr_service, quran_service. Tests in `test_gloss_coverage.py` enforce all three paths call it.
