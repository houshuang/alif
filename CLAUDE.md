...
- **LLM**: LiteLLM for unified multi-model. Sentence generation: Gemini Flash (best quality/compliance/cost). Quality gate: Gemini Flash, fail-closed. General tasks: Gemini 3 Flash primary → GPT-5.2 fallback → Claude Haiku tertiary. Keys: GEMINI_KEY, OPENAI_KEY, ANTHROPIC_API_KEY in `.env`
