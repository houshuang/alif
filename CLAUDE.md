...
See `docs/review-modes.md` for full UX flows. Modes: Sentence-First Review (primary), Reading Mode, Listening Mode, Learn Mode, Story Mode, Quran Reading Mode (suspended 2026-04-07), Podcast Mode. The bilingual Book Reader is deliberately passage-first rather than a review card: opening and drafting are inert; only a successful Next commits exact-range evidence. Clean policy uses Box-2 recognition floors and authoritative rating-1 misses. Guided policy leaves unintroduced words inert behind inline glosses; repeated reading relevance may later select them through ordinary capped Box 1. Under automatic attention v2, clean reading also stages new vocabulary; legacy enrollment buttons cannot override automatic eligibility. Already introduced, maintained taps remain rating-1 misses. Reading support/parked words never schedule from a lookup alone.
...
- **FOUNDATIONAL: Every word in every sentence produces learning evidence** — every non-function word remains visible to the review engine: acquisition and fragile/due FSRS words get normal scheduling credit, while all appearances retain encounter and exact-token evidence. Under `automatic_attention_v2`, non-maintained words preserve token/encounter evidence without scheduling. `automatic_attention.py` selects eligibility using broad non-Quran frequency, authentic reading recurrence and maintenance cost; new external/book vocabulary stages for automatic evaluation (intake remains capped at 0–2/day). There are no learner classification controls or required opt-ins. All operational gates use `attention_policy.py`; identity-QA parked words must never auto-resume. The separate `low_energy_maintenance_v1` exception is: a clean reading-mode **collateral** appearance of a `known`, not-due FSRS word at retrievability ≥0.97 is exposure-only and must not advance its due date. Primary words, due words, red/yellow outcomes, acquiring/learning/lapsed words, listening, and low/unknown retrievability still schedule normally. Encountered words that appear in reviewed sentences are auto-introduced through `start_acquisition()` unless the true-new cap defers them; cap-deferred words keep `encountered`, increment `total_encounters`, and may promote later. During the experiment the cap is 2/day (legacy rollback: 30); `leech_reintro` bypasses it. Do not widen the exposure predicate without a new versioned experiment.
...
- **Recovery permission uses strict debt + primary evidence** — under `low_energy_maintenance_v1`, the true-new recovery budget is `0/1/2` (legacy rollback: `0/8/30`) and activates at actionable/protected Box 1 ≥5, due Box 2 ≥30, or strict main-lane FSRS due ≥750. Under maintenance, a due Box-1 word left unserved through seven learner-active UTC days leaves the Box-1 count (it stays selectable; inactive days never qualify). The FSRS count excludes function/inert rows and variants shadowed by known/learning canonicals. Permission is earned only from primary reading cards/accuracy, never collateral rows or passage child logs. Do not replace this with a raw due count or blended ReviewLog accuracy.
- **A reintroduced leech gets a fresh treatment episode** — lifetime `times_seen`/`times_correct` stay preserved, but suspension evidence for `acquisition_episode_kind='leech_reintro'` begins at `acquisition_started_at` and needs five fresh reviews before a verdict. Reintroduction admission is independently capped at 8/UTC day and closes at due Box 2 ≥30, strict main-lane FSRS due ≥750, or Box-1 load: under `low_energy_maintenance_v1` (v1.1, 2026-09-17) occupancy ≥4 counting not-yet-due rows, so a reintroduction can never by itself trip true-new recovery; legacy rollback uses actionable Box 1 ≥20. Never let pre-treatment failures immediately re-suspend a word after a Good/Tier-E review.
...
- **Passage efficiency is measured at word level** — session and lifetime activity must separate review cards, passage cards, child sentences, Arabic running words, and reviewed-lemma judgments. Red/yellow word rates are the primary passage-versus-sentence outcome; do not promote whole-card “fully understood,” which is mechanically length-confounded. V2 generated passages are 35–45 mapped running words, repeated selected targets must vary mapped surface form, and revealed passages pair each Arabic child sentence with its translation.
...
- **Running-text mapping must retain the full surface and use the surface-aware resolver** — build with `build_comprehensive_lemma_lookup()`, then use `map_tokens_to_lemmas()` for sentences or `lookup_lemma_id(surface, lookup)` for one token. `lookup_lemma()` is a bare-form internal primitive: do not pass it a raw running token after stripping away identity. The surface-aware path applies exact identity before clitic stripping, CAMeL disambiguation, and normalized collision handling. Bare hamzated `أن`/`إن` and prefixed `بأن`/`وإن`/`وأن`/`فأن`/`فإن` are contextual two-identity sets, never fixed aliases; unhamzated `ان`/`فان` are unresolved, exact madda `آن` stays lexical, and `بان` remains lexical. Productive attached-pronoun forms under the approved و/ف/ب prefixes canonicalize to base `أَنَّ`/`إِنَّ` while preserving `SentenceWord.surface_form`; legacy compound lemma rows do not steal those surfaces. Stored lexical `لأنّ` and its pronoun forms beat derived prefix aliases, exact `لِأَنْ` remains `أَنْ`, and unsupported ب+إن compositions fail closed. The only approved exact-running-text aliases are fully vocalized `أُنَاسٌ` → `نَاسٌ` and `فَقَدْ` → `قَدْ`: register them only when the destination identity is unique and gated and no exact source identity exists; preserve the visible surface, replace the bare target identity, and fail closed through CAMeL, proper-name, import, story, OCR/Quran, analysis, and repair fallbacks if any precondition fails. Never generalize them to unvocalized or other case forms. Verifier mutations must go through `apply_corrections()`, which accepts only the already-resolved exact destination for a governed surface; `correct_mapping()` must not learn a broad alias shortcut. Strict citation lookup remains separate and may resolve these exact fully vocalized identities without enabling running-text fuzziness. Target spelling is only a candidate signal: map exact identity first, then recompute target flags after every verifier disambiguation/correction and reject publication if the required canonical target disappeared. Classify each token by the **resolved lemma's** attributes — `_is_function_word(lemma_ar_bare)` and `word_category in {proper_name, onomatopoeia}` — NEVER a surface-only check (it misses clitic-attached forms, e.g. بعضهم→بعض, inflating gaps) or a CAMeL POS guess for proper names (`word_category` is authoritative). Reference impl: `scripts/reading_readiness.py` `analyze()`. Source-specific lexical inputs with independent annotations (for example `quran_frequency.resolve_qac_lemma`) may use the shared bare primitive because they are not running-token surfaces. Don't hand-roll tokenize+normalize+classify for analysis/import/scan work (2026-06-03 lesson).
- **Exact-alias uniqueness is inventory-wide** — exactly one stored destination identity may exist and that sole row must be gated. A gated destination plus an ungated duplicate is ambiguous and must fail closed. Rebuild a lookup after any commit that can create identity rows; never carry its alias metadata across independently committed batch items.
...
- **Authentic corpus enrichment is always scoped, inventory-closed, source-preserving, and two-phase** — use `update_material.py --only-corpus-enrichment` (or the explicitly opted-in full-workflow Step A2) with `--kind` and/or repeatable `--corpus-sentence-id`; kind and IDs intersect. A bounded deterministic preflight advances a scope-specific cursor, exposes prospective mapping/inventory risk before LLM calls, and skips guaranteed-incomplete rows. Preparation performs early Arabic/translation QA, requires an explicit verifier verdict for every row and ambiguity, retries only the exact unavailable/invalid row, requires gated lemmas for both lookup and correction, and uses content + mapping-state compare-and-set writes across every external call so stale verdicts cannot overwrite concurrent text or `SentenceWord` work. A concurrent edit preserves the external work, keeps the row inactive, and independently clears only an unchanged mapping or QA stamp; a translation write also guards the exact Arabic sent to the provider. When tashkīl is missing, provider Arabic is only a harakat proposal: exact NFC content tokens and word boundaries must match, only U+064B–U+0652 may transfer, and the final text is rebuilt from the source so spelling, punctuation, spaces, digits, and tatweel stay authoritative. Letter substitutions (`ى`/`ي`, hamza identity, etc.), word joins/splits, dagger alef, and conflicting/orphan marks retry the row. Enrichment writes Arabic/transliteration only when tashkīl was missing and translation only when translation was missing—provider echoes may never replace an existing field. For a controlled API retry only, `ALIF_CORPUS_ENRICH_PROVIDER={openai,anthropic}` pins that API; unset preserves the normal Codex CLI → Claude CLI → API chain. The one approved non-provider exception is the immutable exact-seven Chapter 1 manifest in `prefill_momo_ch1_curated_2026_07_30.py`: it may CAS-write only independently reviewed Arabic, English, and version-pinned reviewed transliteration after a hash-pinned plan on clean deployed `main` plus a fresh backup whose checksum, Alif schema, exact source preimages, and static live correspondence all verify; it must then re-enter this normal pipeline with activation zero. Never generalize that script into a caller-supplied override or use it for the three excluded rows. Corpus preparation never creates vocabulary—not even proper-name or frequency-core rescue lemmas. Inventory/mapping failures become the durable Jan-2 blocker; completed linguistic QA failures become Jan-3; a target that becomes temporarily suspended during external work is a transient retry and preserves any prior Jan-2 disposition. Only explicitly named Jan-2 rows may be retried with `--corpus-retry-blocked` after separately confirmed source/inventory curation. Successful rows remain inactive. Preparation and activation limits must never both be nonzero: prepare with `--corpus-activate-limit 0`; activate only with `--corpus-limit 0`. Once a `source in {"book", "corpus"}` row has completed QA, **only this governor may activate it**—due-dense salvage and green-page book reactivation exclude it. Activation defaults to zero, is capped at 20, and treats the read-only plan as advisory: one short SQLite writer boundary reloads live capacity plus selected-lemma demand, then planned parent/mapping snapshots and the ceiling are checked before visibility. It blocks acquiring/no-FSRS-demand rows, never retires material, and invalidates stale derived stamps rather than trusting a changed row on the next pass. Never enable corpus cron globally or bulk-reset durable sentinel debt.
- **Published *Momo* provenance is a QA context, never a QA bypass** — `sentence_quality_review_input()` may attach only the allowlisted, versioned `MOMO_PUBLISHED_ARABIC_V1` policy, and only from live row metadata `source="corpus"` plus `kind="momo_book"`. It tells the reviewer that `مُومُو` is the established foreign protagonist name and that translated literary prose may be mildly formal, while explicitly preserving strict grammar, tashkīl, coherence, and English-accuracy checks. The public reviewer splits unlike policy keys into separate calls and rejoins by input index; unknown keys fail before a provider call. Corpus pass/reject writes snapshot and compare source/kind with Arabic/English, and active-review/salvage callers use the same helper. Every verdict-consuming maintenance path must conditionally write against its complete reviewed snapshot; a mismatch retries or skips without overwriting the concurrent edit, and an all-miss salvage batch must explicitly end its SQLite write transaction. Never interpolate free-form provenance or manually stamp a pass.
...
- **Exact-form intervention may select context, never create a form schedule** — `__exact_surface_v1` can reserve at most one already-due reading sentence for the canonical lemma and failed normalized surface. It must not create/advance a card, change the learner's Hard rating, operate in listening/acquisition mode, reuse the trigger sentence, overlap unresolved episodes on one lemma, or compare arms only among exact-form deliveries; preserve the any-form intention-to-treat endpoint and undo integration. The corrective `__form_recovery_v1` ledger is separate: it may prefer at most one already-eligible due-bearing sentence with the recovery form as collateral, but likewise never creates a card/due state or expands a session.
- **Form/tashkeel scheduling protection requires protocol-v3 complete-token proof** — v3 records every exact `SentenceWord`, actual initial/front/back tashkeel exposure, product rating, and optional red/yellow causes. `form_recovery_v1` is the only scheduler protection: on partial reading reviews, every mapped occurrence must validate and every failing token must contain only a structurally meaningful `unfamiliar_form` and/or genuine initially-hidden `missing_tashkeel`. Keep product rating 1/2 but leave the event outside canonical seen/correct counters; apply FSRS Hard or acquisition same-box, exclude the protected row from canonical failure signals, and require two later green tokens from distinct sentences. Any ambiguous, missing, retrieval-lapse, mixed-up, or whole-card evidence is not protected. `mixed_up_total_lapse_v1` is the cause-specific inverse: yellow + validated `mixed_up` preserves product rating 2 and confusion evidence but applies total-lapse scheduling (established assisted FSRS Again; acquisition effective rating 1 / Box 1 reset), even when a form cause is also present; a named confusor is optional. Do not widen either mapping without a new versioned experiment-log entry. Analyses must include successful unvocalized tokens as the denominator and must not infer historical visibility from `show_tashkeel`.
