...
- **Model routing**: `claude_sonnet` for sentence generation (95% naturalness, 1.90 sents/lemma via tool-enabled self-correct session — see 2026-04-20 experiment log), `claude_haiku` for quality gate + enrichment + tagging + flags + disambiguation + verification. Story gen: Claude Opus via `claude_code.py` (retry loop).
- **Sentence generation is self-correcting by default** — `batch_generate_material` (and by delegation `generate_material_for_word`) opens ONE tool-enabled Sonnet CLI session per batch of ~10 lemmas. Sonnet drafts, runs a Bash-invoked `validator.py` in the work_dir, surgically swaps unknown words, and re-validates — no full regenerations. Implementation: `app/services/sentence_self_correct.py` (`generate_sentences_self_correct_batch`). Legacy generate-then-rerank path still available via `ALIF_USE_LEGACY_BATCH=1` for rollback. Benchmark 2026-04-20: N=10 is the quality/cost sweet spot; N=15 regressed to 56% naturalness.
...
- **Every sentence_word must have a lemma_id at *display* time** — two distinct gates:
  ...
  - **Reviewability gate**: every selection query that returns a sentence to the user adds `not_has_unmapped_words()` from `app/services/sentence_eligibility.py`. A sentence with any NULL lemma_id is invisible to the review pipeline until its words are remapped, regardless of `is_active`.
  ...
  - **Proper-name lemmas** are inert: filtered from `word_selector` (no auto-introduction), excluded from comprehensibility scaffold count in `sentence_selector`, and skipped in `sentence_review_service` (no FSRS / acquisition credit). They exist solely so the SentenceWord row carries a real `lemma_id`.
- **No auto-created lemmas from corrections** — `correct_mapping()` and flag resolution only use existing DB lemmas. If the correct lemma isn't in the vocabulary, the sentence is rejected/retired. This prevents orphan lemmas that bypass quality gates. **All correction application must go through `apply_corrections()`** in `sentence_validator.py` — the single shared function for the correct_mapping → 3-way check pattern. Never duplicate this loop inline; the "fix one site, miss the clones" pattern caused 63 bad corpus sentences (2026-04-16).
...
- `sentence_review` / `experiment_intro_shown` / `auto_introduce` / `word_graduated` / `leech_suspended` — ack-driven; fire on user action.
