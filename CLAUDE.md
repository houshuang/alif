...
- **LLM**: All text generation via Claude CLI (free via Max plan). Sonnet for sentence gen (87% pass rate), Haiku for quality gate + enrichment + tagging + flags + disambiguation. Story gen: Claude Opus (retry loop). **Exception**: interactive chat (`/api/chat/ask`) uses Anthropic API directly (Claude Haiku via litellm) — CLI subprocess startup is too slow for real-time user-facing responses. API fallback (when CLI unavailable): GPT-5.2 → Claude Haiku API. Gemini used ONLY for vision/OCR (`ocr_service.py`). Keys: GEMINI_KEY (OCR only), OPENAI_KEY, ANTHROPIC_API_KEY in `.env`
- **Claude Code CLI**: `claude -p` wrapper for free LLM via Max plan. Primary LLM backend for ALL text tasks — integrated into `llm.py` as `claude_sonnet`/`claude_haiku` model overrides (also default when no override specified). Also: standalone `generate_structured()` + `generate_with_tools()` in `claude_code.py`. See `docs/backend-services.md`.
