...
- **Hosting**: Hetzner (46.225.75.29), docker-compose. Backend port 3000→8000. Frontend systemd (`alif-expo`) port 8081. DuckDNS: `alifstian.duckdns.org`. Claude CLI bind-mounted into container (node + claude binary + auth config from host). Limbic bind-mounted at `/opt/limbic` (ro) with cost DB at `/opt/limbic-data/llm_costs.db`.
