...
- `word_selector.py` — Next-word algorithm: 40% freq + 30% root + 20% recency + 10% grammar + encountered/story bonus + page bonus + category penalty (proper_name: -0.8, onomatopoeia: -1.0). Root-sibling interference guard. Excludes wiktionary refs and variant lemmas. introduce_word() calls start_acquisition(), accepts `due_immediately` param for auto-introduction during sessions. Auto-introduction (`sentence_selector._auto_introduce_words`) never picks proper_name or onomatopoeia words. **Book page bonus**: page 1→1.5, page 2→1.0, page 3→0.6, page 4→0.3, page 5+→0.1 — strongly prioritizes early pages so learner can read page 1 first. **Topic filter bypass**: words from active stories always pass the topic filter, so book vocabulary is prioritized over topic rotation.
