...
- `material_generator.py` — Orchestrates sentence + audio generation for a word. Dynamic difficulty via `get_sentence_difficulty_params()`. Default needed=2 (warm cache), requests needed+2 to absorb validation failures. JIT on-demand generation in session builder uses current vocabulary for fresher sentences. `store_multi_target_sentence()` for saving multi-target generated sentences with correct SentenceWord mappings. `warm_sentence_cache()` — background task that pre-generates sentences for focus cohort gaps + likely auto-introduction candidates; respects 300-sentence pipeline cap. **NULL lemma_id guard**: all sentence storage paths reject sentences with any unmapped word (uses `build_comprehensive_lemma_lookup()` for mapping).
