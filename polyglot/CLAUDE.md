...
| Intro card filter | **Deferred** | Will land with intro-card port (see Hard Invariant #12). |
| Comprehensibility gate | **Ported (picker-side)** | `sentence_selector._score_candidate` weights candidates by fraction-of-scaffold-known and applies a strong page-first bonus only when ALL content scaffold lemmas are in `known`/`learning` state. Generation-side comprehensibility still deferred (PR #4). |
